...
import {
  BATCH_SIZE,
  COLLECTION_NAME,
  prepareTypesenseEmpiresData,
  TYPESENSE_EMPIRE_SCHEMA,
} from "./schema/empire";

export const indexTypesenseEmpires = async () => {
  const INDEX_NAME = `${COLLECTION_NAME}_${Date.now()}`;
  const documents = await prepareTypesenseEmpiresData();
  ...
  await client.collections().create(TYPESENSE_EMPIRE_SCHEMA(INDEX_NAME));
